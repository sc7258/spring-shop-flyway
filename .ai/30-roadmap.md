...
- [x] **MariaDB Migration**
  - [x] PostgreSQL 의존성과 로컬 실행 가이드를 MariaDB 기준으로 전환
  - [x] Flyway 마이그레이션과 JPA 설정이 MariaDB에서 동일하게 동작하도록 검증
  - [x] Docker Compose 및 개발용 DB 부트스트랩을 MariaDB 기준으로 재구성
- [x] **Environment Profile Strategy**
  - [x] `local` 프로필 제거
  - [x] `dev`, `qa`, `prod` 3단계 프로필로 재구성
  - [x] 환경별 datasource / logging / security 설정 차이를 문서화
- [x] **Persistence Consistency**
  - [x] `created_at`, `updated_at` 컬럼 관리를 Spring Data JPA Auditing 기반으로 표준화
  - [x] 엔티티별 수동 timestamp 갱신 로직을 제거하고 공통 audit 모델로 정리
  - [x] 저장/수정 시각 회귀 테스트를 추가해 MariaDB/H2 양쪽에서 일관성을 검증
...
- [x] **Performance Tuning**
  - [x] N+1 문제 해결 및 인덱싱 최적화
  - [x] 성능 테스트 수행 및 병목 지점 개선
